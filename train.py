...
def train_epoch(
    ...
    window_winner_counts = torch.zeros(
    ...
    for batch in batches:
        ...
        with torch.amp.autocast(
            ...
            (
                ...
                trajectories,
                confidence_logits,
            ) = training_losses(predictor, batch)
        ...
        non_finite_total_count += int(
            not math.isfinite(component_values["total"])
        )
...
def main():
    ...
    parser.add_argument(
        "--stop-after-seconds", type=float, required=True
    )
    ...
    optimizer = torch.optim.AdamW(
        parameter_groups(predictor), lr=arguments.learning_rate
    )
    ...
    if arguments.resume and resume_path.exists():
        checkpoint = model.load_checkpoint_state(
        ...
        if "gradient_scaler_state" in checkpoint:
            gradient_scaler.load_state_dict(
    ...
    remaining_epochs = epochs_left_to_train(
    ...
    for epoch_index in remaining_epochs:
        ...
        if (
            ...
            print(
                ...
                f" {epoch_index} of {arguments.epochs} epochs are complete and"
