...
def synthetic_scene_batch(sample_count, neighbour_count, polyline_count,
                          dots_per_polyline):
    ...
    return {
        ...
        "max_chunks_in_batch": torch.tensor(polyline_count),
...
def test_input_order_does_not_change_predictions():
    """Neighbours and map dots are shuffled; trajectories must not move."""
    ...
    permuted["neighbour_history_mask"] = batch[
        "neighbour_history_mask"][:, neighbour_order]
    permuted["neighbour_signal_history"] = batch[
        "neighbour_signal_history"][:, neighbour_order]
