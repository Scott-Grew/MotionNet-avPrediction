...
STAGED_DIRECTORY = (
    Path(__file__).resolve().parents[2] / "data" / "staged"
)


def lane_polyline_rows(first_dot_x, dot_count):
    rows = np.zeros(
        (dot_count, contract.MAP_FEATURE_DIM), dtype=np.float32
    )
...
def synthetic_scene_batch(
    sample_count, neighbour_count, polyline_count, dots_per_polyline
):
    ...
    map_rows[:, contract.MAP_LEFT_BOUNDARY_CROSSING :] = (
        torch.randint(
            0,
    ...
    return {
        ...
        "map_rows": map_rows,
        "map_dot_polyline_slot": torch.arange(dot_count)
        // dots_per_polyline,
...
def test_v1_agent_frame_transform_inverts():
    ...
    world_positions = random_generator.uniform(
        -120.0, 120.0, size=(64, 2)
    )
    origin = np.array([13.5, -42.25])
    heading = 0.9137

    local = frame_ops.positions_to_agent_frame(
        world_positions, origin, heading
    )
    recovered = frame_ops.positions_to_world_frame(
...
def test_v1_the_agent_frame_puts_a_point_straight_ahead_on_its_own_forward_axis():
    ...
    assert np.allclose(
        straight_ahead, np.array([distance, 0.0]), atol=1e-9
    )
    assert np.allclose(
        off_to_the_left, np.array([0.0, distance]), atol=1e-9
    )

    transposed_convention = (
        distance * forward
    ) @ frame_ops.rotation_matrix(heading).T
    assert not np.allclose(
        transposed_convention, straight_ahead, atol=1e-9
    )
...
def test_v6_storage_then_agent_frame_matches_direct_world_to_agent():
    ...
    two_step = loader.track_rows_to_agent_frame(
        track_rows[0], origin, heading
    )

    world_positions = np.array(
        [[state.center_x, state.center_y] for state in track.states]
    )
    world_headings = np.array(
        [state.heading for state in track.states]
    )
    ...
    direct_positions = frame_ops.positions_to_agent_frame(
        world_positions, now_position, now_state.heading
    )
    direct_headings = world_headings - now_state.heading
    direct_velocities = frame_ops.directions_to_agent_frame(
        world_velocities, now_state.heading
    )
...
def test_polyline_pooling_isolates_groups_and_leaves_empty_slots_absent():
    ...
    tokens, present = model.pool_dots_to_polyline_tokens(
        dot_embeddings, dot_polyline_slot, 2, 3
    )
    ...
    poisoned[:3] = 999.0
    poisoned_tokens, _ = model.pool_dots_to_polyline_tokens(
...
def test_v4_null_baselines_reproduce_the_motion_each_one_assumes():
    step_offsets = np.arange(
        -contract.CURRENT_STEP_INDEX, contract.FUTURE_STEPS + 1
    )
    ...
    agent_track[..., contract.AGENT_VELOCITY] = speeds[
        :, None, None
    ] * np.stack([np.cos(turned), np.sin(turned)], axis=-1)
    ...
    assert torch.allclose(
        turning_trajectories[:, 0], logged_future, atol=1e-3
    )
    assert (
        straight_trajectories[0, 0, -1] - logged_future[0, -1]
    ).norm() > 1.0
    ...
    assert torch.equal(
        pruned,
        turning_trajectories.expand(
            -1, contract.NUM_PREDICTED_MODES, -1, -1
        ),
    )
...
def test_v4_anchor_null_drives_straight_to_each_most_used_anchor_in_metres_regardless_of_speed():
    ...
    agent_history = torch.zeros(
        2, contract.HISTORY_STEPS, contract.AGENT_FEATURE_DIM
    )
    agent_history[
        :, contract.CURRENT_STEP_INDEX, contract.AGENT_VELOCITY
    ] = torch.stack([current_speeds, torch.zeros(2)], dim=-1)
    agent_history[
        torch.arange(2),
    ...
    trajectories, confidence_logits = (
        baseline.straight_lines_to_most_used_anchors(
            batch, unit_anchors
        )
    )
    ...
    steps = torch.cat(
        [trajectories[..., :1, :], trajectories.diff(dim=-2)], dim=-2
    )
    assert torch.allclose(
        steps, steps[..., :1, :].expand_as(steps), atol=1e-5
    )
...
def test_anchor_fitting_recovers_tight_well_separated_clusters_and_leaves_no_anchor_empty():
    ...
    cluster_centres = (
        model.unit_anchor_offsets() * 10.0
        + torch.tensor(
            random_generator.uniform(
                -0.08, 0.08, (model.QUERY_COUNT, 2)
            ),
            dtype=torch.float32,
        )
    )
    endpoints = (
        ...
        + torch.tensor(
            random_generator.normal(
                0.0, 0.005, (model.QUERY_COUNT, 50, 2)
            ),
            dtype=torch.float32,
        )
    ).reshape(-1, 2)
    ...
    nearest_cluster = torch.cdist(fitted, cluster_centres).argmin(
        dim=1
    )
    assert sorted(nearest_cluster.tolist()) == list(
        range(model.QUERY_COUNT)
    )
    assert torch.allclose(
        fitted, cluster_centres[nearest_cluster], atol=0.01
    )
...
def test_batched_pruning_walk_matches_the_single_sample_walk():
    ...
    batched_trajectories, batched_logits, kept_counts = (
        model.prune_modes_batched_with_kept_count(
            trajectories, confidence_logits
        )
    )
    assert (
        kept_counts[0] > 1
        and kept_counts[0] <= contract.NUM_PREDICTED_MODES
    )
    ...
    for sample_index in range(len(kept_counts)):
        ...
        assert torch.equal(
            walked_trajectories, batched_trajectories[sample_index]
        )
        assert torch.equal(
            walked_logits, batched_logits[sample_index]
        )
...
def test_opposed_logged_endpoints_are_assigned_to_opposed_anchor_territories():
    ...
    future_positions[0, :, 0] = torch.linspace(
        0.5, 40.0, contract.FUTURE_STEPS
    )
    future_positions[1, :, 0] = torch.linspace(
        -0.5, -40.0, contract.FUTURE_STEPS
    )
    future_mask = torch.ones(
    ...
    assert (
        ...
        - unit_anchors[assigned_mode[1]]
    ).norm() > 20.0


def test_assignment_scores_the_whole_valid_path_and_ignores_the_padded_tail():
    unit_anchors = model.unit_anchor_offsets() * 40.0
    future_positions = torch.zeros(1, contract.FUTURE_STEPS, 2)
    future_positions[0, :10, 0] = 40.0
    future_mask = torch.zeros(
        1, contract.FUTURE_STEPS, dtype=torch.bool
    )
    ...
    assert float(
        unit_anchors[padded_tail_mode[0]].norm()
    ) == pytest.approx(40.0 / model.ANCHOR_DISTANCE_COUNT)
...
def test_submission_world_frame_returns_the_logged_future_to_its_logged_place():
    ...
    world_future = submit.agent_frame_to_world_frame(
        agent_frame_future, sample, scenario_array
    )
    world_coordinate_resolution = (
        np.finfo(np.float32).eps * np.abs(world_future).max()
    )
    ...
    logged = scenario_array["track_valid"][
...
def test_the_backfill_monitor_tells_six_distinct_futures_from_one_future_repeated():
    ...
    assert separated_accumulator.results()[
        "mean_kept_modes"
    ] == float(contract.NUM_PREDICTED_MODES)
...
def test_v4_the_constant_velocity_null_carries_a_straight_agent_at_exactly_its_logged_speed():
    ...
    agent_history[
        :, contract.CURRENT_STEP_INDEX, contract.AGENT_POSITION
    ] = torch.tensor([[3.0, 5.0], [3.0, 5.0]])
    batch = {
        "agent_history": agent_history,
        "agent_history_mask": torch.ones(
            2, contract.HISTORY_STEPS, dtype=torch.bool
    ...
    assert torch.allclose(
        trajectories[0, 0], straight_line, atol=1e-4
    )
    assert torch.allclose(
...
def test_artifact_provenance_round_trips_and_refuses_missing_or_mismatched_stamps():
    stamp = contract.artifact_provenance(
        "test_invariants.py", "unit-test-source"
    )
    checked = contract.check_artifact_provenance(
        stamp, "in-memory", "Regenerate."
    )
    ...
    with pytest.raises(AssertionError):
        contract.check_artifact_provenance(
            stale, "in-memory", "Regenerate."
        )


def test_scorer_tensors_group_per_scenario_with_every_agent_in_the_ground_truth(
    tmp_path,
):
    ...
    for scenario_id, agent_count in scenario_agent_counts.items():
        ...
        for track_index in range(agent_count):
            track_rows[
                track_index, :, contract.AGENT_POSITION.start
            ] = float(track_index)
        np.savez(
            tmp_path / f"{scenario_id}.npz",
            track_rows=track_rows,
            track_valid=np.ones(
                (agent_count, contract.TOTAL_STEPS), dtype=bool
            ),
            track_ids=np.arange(
                10, 10 * (agent_count + 1), 10, dtype=np.int64
            ),
            is_designated_target=np.ones(agent_count, dtype=bool),
            is_object_of_interest=np.zeros(agent_count, dtype=bool),
            map_rows=np.zeros(
                (0, contract.MAP_FEATURE_DIM), dtype=np.float32
            ),
    ...
    predictions = {
        ...
        "confidences": np.full(
            (target_count, contract.NUM_PREDICTED_MODES), 1.0 / 6.0
        ),
    }

    tensors = runner.build_motion_metric_tensors(
    ...
    assert (
        int(tensors["prediction_ground_truth_indices_mask"].sum())
        == target_count
    )
    ...
    assert not tensors["prediction_ground_truth_indices_mask"][
        b_row, 1:
    ].any()
    ...
    for slot_index, expected_track_id in enumerate(
        ...
        agent_row = int(
            tensors["prediction_ground_truth_indices"][
                a_row, slot_index, 0
            ]
        )
        assert (
            tensors["object_id"][a_row, agent_row]
            == expected_track_id
        )
...
def test_permuting_neighbours_and_map_dots_leaves_trajectories_unchanged():
    ...
    assert torch.allclose(
        base_trajectories, permuted_trajectories, atol=1e-4
    )
...
def test_each_agent_carries_the_signal_history_of_the_lane_it_is_assigned_to():
    ...
    signalled_lane_history[
        :8, contract.TRAFFIC_SIGNAL_STATES.index("LANE_STATE_STOP")
    ] = 1.0
    signalled_lane_history[
...
def test_a_mode_endpoint_is_its_anchor_when_the_head_is_zero_and_the_loss_assigns_by_that_anchor():
    ...
    with torch.no_grad():
        trajectories, _, _, selected_unit_anchors = predictor.predict(
            batch
        )
    assert torch.allclose(
        trajectories[:, :, -1], unit_anchors, atol=1e-4
    )
    assert torch.allclose(selected_unit_anchors, unit_anchors)
    ramp = (
        torch.arange(1, contract.FUTURE_STEPS + 1)
        / contract.FUTURE_STEPS
    )
    ...
    assigned_mode = loss.anchor_assigned_mode(
        ...
        logged_futures,
        torch.ones(
            model.QUERY_COUNT, contract.FUTURE_STEPS, dtype=torch.bool
        ),
    )
...
def test_the_regression_term_is_the_gaussian_negative_log_likelihood_at_the_stated_uncertainty():
    ...
    unit_anchors = model.unit_anchor_offsets().expand(
        sample_count, -1, -1
    )
    ...
    def regression_at(log_standard_deviation, error):
        trajectories = (
            future_positions.unsqueeze(1).expand(
                -1, model.QUERY_COUNT, -1, -1
            )
            + error
        )
    ...
    assert regression_at(floor, displacement) == pytest.approx(
        exact_form, rel=1e-5
    )
    for log_standard_deviation in (
        ...
        assert regression_at(
            log_standard_deviation, torch.zeros(2)
        ) < regression_at(log_standard_deviation, displacement)
...
def test_classification_is_softmax_cross_entropy_on_the_assigned_mode():
    ...
    assert float(classification) == pytest.approx(
        math.log(model.QUERY_COUNT), rel=1e-5
    )
...
def test_a_masked_future_step_moves_no_term_of_the_loss():
    ...
    confidence_logits = torch.randn(sample_count, model.QUERY_COUNT)
    unit_anchors = (
        model.unit_anchor_offsets().expand(sample_count, -1, -1)
        * 40.0
    )
    ...
    poisoned_trajectories[
        masked_steps.unsqueeze(1).expand(-1, model.QUERY_COUNT, -1)
    ] = 1e6
    assert torch.equal(
...
def test_no_step_uncertainty_starts_saturated_and_untrained_modes_are_distinct():
    ...
    assert (
        trajectories.isfinite().all()
        and confidence_logits.isfinite().all()
    )
    assert not (
        log_standard_deviation == model.MINIMUM_LOG_STANDARD_DEVIATION
    ).any()
    assert not (
        log_standard_deviation == model.MAXIMUM_LOG_STANDARD_DEVIATION
    ).any()
...
def test_one_training_step_runs_the_whole_path_over_staged_scenarios(
    tmp_path,
):
    ...
    batch = next(
        iter(pipeline.batches(scenario_paths, 0, 2, 0, 0, True))
    )
    (
        total,
        regression,
    ...
    assert (
        torch.stack([total, regression, classification])
        .isfinite()
        .all()
    )
    total.backward()
    ...
    assert all(
        math.isfinite(value)
        for value in accumulator.results().values()
    )
...
def test_submitted_confidences_conserve_probability_mass_and_hand_a_duplicates_share_to_its_keeper():
    ...
    trajectories = torch.zeros(
        1, model.QUERY_COUNT, contract.FUTURE_STEPS, 2
    )
    trajectories[0, :, -1, 0] = 10.0 * torch.arange(
        model.QUERY_COUNT, dtype=torch.float32
    )
    ...
    kept_trajectories, kept_logits = model.prune_modes_batched(
        trajectories, confidence_logits
    )
    ...
    assert float(confidences[0, 0]) == pytest.approx(
        float(probabilities[0] + probabilities[1]), rel=1e-6
    )
...
def test_a_constant_step_head_walks_a_straight_line_and_a_rotating_step_head_walks_an_arc():
    ...
    decoder = model.ModeDecoder(
        model.unit_anchor_offsets_per_type() * 0.0
    ).eval()
    ...
    with torch.no_grad():
        ...
        bias = decoder.trajectory_head[-1].bias.view(
            2, contract.FUTURE_STEPS, 2
        )
        bias.zero_()
        bias[0, :, 0] = 0.5
        straight, _, _, _ = decoder(
            tokens, token_present, torch.zeros(1, dtype=torch.long)
        )
        angles = 0.02 * torch.arange(
            contract.FUTURE_STEPS, dtype=torch.float32
        )
    ...
    assert torch.allclose(
        straight[0, 0], expected_straight, atol=1e-5
    )
    ...
    assert torch.allclose(
        turning, torch.full_like(turning, 0.02), atol=1e-5
    )
