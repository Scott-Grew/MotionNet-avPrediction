...
def lane_polyline_rows(first_dot_x, dot_count):
    ...
    rows[:, contract.MAP_POSITION] = np.stack(
        [
            first_dot_x + np.arange(dot_count, dtype=np.float64),
            np.zeros(dot_count),
        ],
        axis=1,
    )
    ...
    rows[
        :,
        contract.MAP_KIND.start + contract.MAP_POLYLINE_KINDS.index("lane"),
    ] = 1.0
    return rows
...
def synthetic_scene_batch(sample_count, scene_agent_count, polyline_count,
    ...
    for sample_index in range(sample_count):
        agent_history[
            sample_index,
            :,
            contract.AGENT_TYPE.start +
            sample_index % contract.NUM_OBJECT_TYPES,
        ] = 1.0
    ...
    map_rows[:, contract.MAP_LEFT_BOUNDARY_CROSSING:] = torch.randint(
        0,
        contract.NUM_BOUNDARY_CROSSING_CODES,
        (dot_count, 2),
        dtype=torch.float32,
    )
    return SceneBatch(
        ...
        map=MapArrays(
            rows=map_rows,
            dot_chunk_slot=torch.arange(dot_count) // dots_per_polyline,
            chunk_signal_history=torch.rand(
                sample_count,
                polyline_count,
                contract.HISTORY_STEPS,
                contract.NUM_TRAFFIC_SIGNAL_STATES,
        ...
        targets=TargetArrays(
            scene_index=torch.arange(sample_count),
            ...
            future_positions=torch.randn(sample_count, contract.FUTURE_STEPS,
                                         2),
...
def test_agent_frame_transform_inverts():
    ...
    local = frame_ops.positions_to_frame(world_positions, origin, heading)
    recovered = frame_ops.positions_from_frame(local, origin, heading)
    assert np.allclose(recovered, world_positions, atol=1e-9)


def test_agent_frame_x_axis_points_ahead():
    """A point straight ahead of the agent lands on its forward axis."""
    origin = np.array([13.5, -42.25])
    heading = 0.9137
    ...
    straight_ahead = frame_ops.positions_to_frame(origin + distance * forward,
                                                  origin, heading)
    off_to_the_left = frame_ops.positions_to_frame(origin + distance * leftward,
                                                   origin, heading)
    ...
    transposed_convention = (distance *
                             forward) @ frame_ops.rotation_matrix(heading).T
...
def test_frame_transforms_compose():
    """World to scene to agent must equal world to agent directly."""
    ...
    direct_positions = frame_ops.positions_to_frame(world_positions,
                                                    now_position,
                                                    now_state.heading)
    direct_headings = world_headings - now_state.heading
    direct_velocities = frame_ops.directions_to_frame(world_velocities,
                                                      now_state.heading)

    assert np.allclose(
        two_step[:, contract.AGENT_POSITION],
        direct_positions,
        atol=1e-3,
    )
    assert np.allclose(
        two_step[:, contract.AGENT_HEADING_COSINE],
        np.cos(direct_headings),
        atol=1e-3,
    )
    assert np.allclose(
        two_step[:, contract.AGENT_HEADING_SINE],
        np.sin(direct_headings),
        atol=1e-3,
    )
    assert np.allclose(
        two_step[:, contract.AGENT_VELOCITY],
        direct_velocities,
        atol=1e-3,
    )


def test_chunk_pooling_isolates_groups():
    """Empty chunk slots must come back marked absent."""
    torch.manual_seed(5)
...
def test_null_baselines_reproduce_their_motion():
    ...
    radii = np.where(
        yaw_rates == 0.0,
        0.0,
        speeds / np.where(yaw_rates == 0.0, 1.0, yaw_rates),
    )
    arc_positions = np.stack(
        [
            radii[:, None] * np.sin(turned),
            radii[:, None] * (1.0 - np.cos(turned)),
        ],
        axis=-1,
    )

    agent_track = np.zeros(
        (2, len(step_offsets), contract.AGENT_FEATURE_DIM),
        dtype=np.float32,
    )
    ...
    pruned, _ = pruning.prune_modes_batched(turning_trajectories,
                                            turning_logits)
    assert torch.equal(
        pruned,
        turning_trajectories.expand(-1, contract.NUM_PREDICTED_MODES, -1, -1),
    )


def test_anchor_null_ends_on_the_most_used_anchors():
    """Anchors are in metres, so the agent's speed must not change where the
    null's paths end.
    """
    unit_anchors = (reference_implementations.unit_anchor_offsets_per_type() *
                    torch.tensor([40.0, 24.0, 12.0])[:, None, None])
    driven_types = torch.tensor([0, contract.NUM_OBJECT_TYPES - 1])
    current_speeds = torch.tensor([8.0, 0.0])
    agent_history = torch.zeros(2, contract.HISTORY_STEPS,
                                contract.AGENT_FEATURE_DIM)
    agent_history[:, contract.CURRENT_STEP_INDEX,
                  contract.AGENT_VELOCITY] = (torch.stack(
    ...
    assert confidence_logits.shape == (
        2,
        contract.NUM_PREDICTED_MODES,
    )
    ...
    steps = torch.cat([trajectories[..., :1, :],
                       trajectories.diff(dim=-2)],
                      dim=-2)
    assert torch.allclose(steps, steps[..., :1, :].expand_as(steps), atol=1e-5)


def test_anchor_fitting_recovers_separated_clusters():
    """No anchor may be left without endpoints."""
    random_generator = np.random.default_rng(17)
    cluster_centres = (reference_implementations.unit_anchor_offsets() * 10.0 +
                       torch.tensor(
                           random_generator.uniform(-0.08, 0.08,
                                                    (model.QUERY_COUNT, 2)),
                           dtype=torch.float32,
                       ))
    endpoints = (cluster_centres[:, None, :] + torch.tensor(
        random_generator.normal(0.0, 0.005, (model.QUERY_COUNT, 50, 2)),
        dtype=torch.float32,
    )).reshape(-1, 2)

    anchor_fit = fit_anchors.fit_unit_anchors(endpoints)
    fitted = anchor_fit.centres
    counts = fit_anchors.endpoints_per_centre(anchor_fit.assignment,
                                              model.QUERY_COUNT)

    assert anchor_fit.stopped_by_convergence
...
def test_batched_pruning_matches_the_loop():
    """The plain single-sample loop is the reference."""
    torch.manual_seed(7)
    trajectories = pruning.PRUNE_DISTANCE_METRES * torch.randn(
    ...
    batched_trajectories, batched_logits, kept_counts = (
        pruning.prune_modes_batched_with_kept_count(trajectories,
                                                    confidence_logits))
    ...
    for sample_index in range(len(kept_counts)):
        walked_trajectories, walked_logits = (
            reference_implementations.prune_modes_single_sample(
                trajectories[sample_index],
                confidence_logits[sample_index],
            ))
        assert torch.equal(walked_trajectories,
                           batched_trajectories[sample_index])
        assert torch.equal(walked_logits, batched_logits[sample_index])


def test_opposite_endpoints_get_opposite_anchors():
    """A future ending ahead and one ending behind are assigned anchors on those
    sides.
    """
    unit_anchors = reference_implementations.unit_anchor_offsets() * 40.0
    future_positions = torch.zeros(2, contract.FUTURE_STEPS, 2)
    future_positions[0, :, 0] = torch.linspace(0.5, 40.0, contract.FUTURE_STEPS)
    future_positions[1, :, 0] = torch.linspace(-0.5, -40.0,
                                               contract.FUTURE_STEPS)
    future_mask = torch.ones(2, contract.FUTURE_STEPS, dtype=torch.bool)

    assigned_mode = loss.anchor_assigned_mode(unit_anchors.expand(2, -1, -1),
    ...
    assert (unit_anchors[assigned_mode[0]] -
            unit_anchors[assigned_mode[1]]).norm() > 20.0


def test_assignment_ignores_padded_steps():
    """Assignment scores the whole valid path, never the padded tail."""
    unit_anchors = reference_implementations.unit_anchor_offsets() * 40.0
    ...
    assigned_mode = loss.anchor_assigned_mode(unit_anchors.expand(1, -1, -1),
                                              future_positions, future_mask)
    padded_tail_mode = loss.anchor_assigned_mode(
        unit_anchors.expand(1, -1, -1),
        future_positions,
        torch.ones_like(future_mask),
    )

    assert torch.allclose(
        unit_anchors[assigned_mode[0]],
    ...
    assert float(unit_anchors[padded_tail_mode[0]].norm()) == pytest.approx(
        40.0 / reference_implementations.ANCHOR_DISTANCE_COUNT)

    waits_then_arrives = torch.zeros(1, contract.FUTURE_STEPS, 2)
    waits_then_arrives[0, -1, 0] = 40.0
    late_mode = loss.anchor_assigned_mode(
        unit_anchors.expand(1, -1, -1),
        waits_then_arrives,
        torch.ones_like(future_mask),
    )
    assert float(unit_anchors[late_mode[0]].norm()) < 40.0


def test_submission_returns_to_the_world_frame():
    """The logged future, sent through the submission transform, must land where
    it was logged.
    """
    ...
    assert np.allclose(
        back_to_agent_frame,
        agent_frame_future,
        atol=world_coordinate_resolution,
    )
    ...
    assert np.allclose(
        world_future[logged],
        logged_world_future[logged],
        atol=world_coordinate_resolution,
    )


def test_backfill_monitor_detects_a_repeated_future():
    """Six distinct futures read as no backfill; one future repeated six times
    reads as full backfill.
    """
    ...
    separated_trajectories = torch.zeros(sample_count, model.QUERY_COUNT,
                                         contract.FUTURE_STEPS, 2)
    separated_trajectories[:, :, -1, 0] = (
        2.0 * pruning.PRUNE_DISTANCE_METRES *
        torch.arange(1, model.QUERY_COUNT + 1, dtype=torch.float32))
...
def test_constant_velocity_null_keeps_the_logged_speed():
    """A straight-moving agent is carried at exactly its logged velocity."""
    logged_velocity = torch.tensor([[11.0, -4.0], [0.0, 0.0]])
    agent_history = torch.zeros(2, contract.HISTORY_STEPS,
                                contract.AGENT_FEATURE_DIM)
    agent_history[:, :, contract.AGENT_HEADING_COSINE] = 1.0
    agent_history[:, contract.CURRENT_STEP_INDEX,
                  contract.AGENT_VELOCITY] = (logged_velocity)
    agent_history[:, contract.CURRENT_STEP_INDEX,
    ...
    straight_line = (torch.tensor([3.0, 5.0]) +
                     logged_velocity[0] * elapsed_seconds[:, None])
    ...
    assert torch.allclose(
        ...
        torch.full(
            (contract.FUTURE_STEPS - 1,),
            float(logged_velocity[0].norm() * baseline.TIMESTEP_SECONDS),
        ),
        atol=1e-4,
    )
    assert torch.allclose(
        trajectories[1, 0],
        torch.tensor([3.0, 5.0]).expand(contract.FUTURE_STEPS, 2),
        atol=1e-4,
    )


def test_artifact_provenance_round_trips():
    """A missing stamp, or one from another code version, is refused."""
    stamp = contract.artifact_provenance("test_invariants.py",
                                         "unit-test-source")
    checked = contract.check_artifact_provenance(stamp, "in-memory",
                                                 "Regenerate.")
    ...
    stale = json_module.dumps({
        "code_version": "not-the-working-tree",
        "producer": "x",
        "source": "y",
    })
    with pytest.raises(AssertionError):
...
def test_scorer_tensors_group_by_scenario(tmp_path,):
    """Ground truth holds every agent, not only the predicted ones."""
    import importlib.util

    runner_spec = importlib.util.spec_from_file_location(
        "container_runner",
        Path(__file__).parent.parent / "container" / "runner.py",
    )
    ...
    for scenario_id, agent_count in scenario_agent_counts.items():
        ...
        for track_index in range(agent_count):
            track_rows[track_index, :,
                       contract.AGENT_POSITION.start] = float(track_index)
        np.savez(
            tmp_path / f"{scenario_id}.npz",
            track_rows=track_rows,
            track_valid=np.ones((agent_count, contract.TOTAL_STEPS),
                                dtype=bool),
    ...
    for (
            scenario_id,
            target_track_ids,
    ) in scenario_target_track_ids.items():
...
def test_input_order_does_not_change_predictions():
    """Scene agents and map dots are shuffled; trajectories must not move."""
    ...
    map_order = torch.randperm(batch.map.rows.shape[0])
    permuted = SceneBatch(
        scene=SceneArrays(
            **{
                name: array[:, scene_agent_order]
                for name, array in batch.scene._asdict().items()
            }),
        map=batch.map._replace(
            rows=batch.map.rows[map_order],
            dot_chunk_slot=batch.map.dot_chunk_slot[map_order],
        ),
        targets=batch.targets._replace(
            token_visible=batch.targets.token_visible[:, token_order],
            token_pose=batch.targets.token_pose[:, token_order],
        ),
    )
    with torch.no_grad():
...
def test_agents_carry_their_lane_signal_history():
    ...
    signalled_lane_history[:8,
                           contract.TRAFFIC_SIGNAL_STATES.
                           index("LANE_STATE_STOP")] = 1.0
    ...
    assert batch.scene.agent_signal_history.shape == (
        2,
        3,
        contract.HISTORY_STEPS,
        contract.NUM_TRAFFIC_SIGNAL_STATES,
    )
    assert np.all(batch.scene.agent_signal_history[1, 2] == 0.0)
    tensor_batch = pipeline.torch_batch(batch)
    silenced_batch = tensor_batch._replace(
        targets=tensor_batch.targets._replace(
            agent_signal_history=torch.zeros_like(
                tensor_batch.targets.agent_signal_history)))
...
def test_target_prediction_ignores_batch_company():
    """A target predicted alone must equal the same target batched beside other
    targets and another scene.
    """
    signalled_lane_history = np.zeros(
        (contract.HISTORY_STEPS, contract.NUM_TRAFFIC_SIGNAL_STATES),
        dtype=np.float32,
    ...
    with torch.no_grad():
        alone, _ = predictor(
            torch_batch([loader.build_scene_sample(three_track_scenario, [1])]))
        together, _ = predictor(
            torch_batch([
                loader.build_scene_sample(two_track_scenario, [0]),
                loader.build_scene_sample(three_track_scenario, [0, 1, 2]),
            ]))
    assert torch.allclose(alone[0], together[2], atol=1e-4)


def test_zero_head_output_lands_on_the_anchor():
    """With a zero head a mode ends at its anchor, and the loss assigns by that
    same anchor.
    """
    ...
    assigned_mode = loss.anchor_assigned_mode(
        unit_anchors[0].expand(model.QUERY_COUNT, -1, -1),
        logged_futures,
...
def test_regression_term_is_gaussian_nll():
    """Checked against the closed form at the uncertainty the model states."""
    torch.manual_seed(53)
    sample_count = 4
    ...
    unit_anchors = reference_implementations.unit_anchor_offsets().expand(
        sample_count, -1, -1)
    ...
    exact_form = (2.0 * floor + 2.0 * loss.HALF_LOG_TWO_PI + 0.5 * float(
        (displacement**2).sum()) / math.exp(floor)**2)
    assert regression_at(floor, displacement) == pytest.approx(exact_form,
                                                               rel=1e-5)
    for log_standard_deviation in (
            floor,
            0.0,
            model.MAXIMUM_LOG_STANDARD_DEVIATION,
    ):
        assert regression_at(log_standard_deviation,
                             torch.zeros(2)) < regression_at(
                                 log_standard_deviation, displacement)


def test_classification_term_is_cross_entropy():
    """Taken over anchors, against the assigned mode."""
    sample_count = 3
    future_positions = torch.zeros(sample_count, contract.FUTURE_STEPS, 2)
    ...
    future_mask = torch.ones(sample_count,
                             contract.FUTURE_STEPS,
                             dtype=torch.bool)
    trajectories = future_positions.unsqueeze(1).expand(-1, model.QUERY_COUNT,
                                                        -1, -1)
    unit_anchors = torch.zeros(sample_count, model.QUERY_COUNT, 2)
    unit_anchors[:, 0, 0] = 1.0
    _, _, classification = loss.prediction_loss(
        trajectories,
        torch.zeros_like(trajectories),
        torch.zeros(sample_count, model.QUERY_COUNT),
        future_positions,
        future_mask,
        unit_anchors,
    )
    assert float(classification) == pytest.approx(math.log(model.QUERY_COUNT),
                                                  rel=1e-5)
...
def test_masked_steps_move_no_loss():
    ...
    future_mask[2] = False
    future_positions = torch.randn(sample_count, contract.FUTURE_STEPS, 2)
    trajectories = torch.randn(sample_count, model.QUERY_COUNT,
                               contract.FUTURE_STEPS, 2)
    ...
    poisoned_trajectories[masked_steps.unsqueeze(1).expand(
        -1, model.QUERY_COUNT, -1)] = 1e6
    assert torch.equal(
        components(future_positions, trajectories),
        components(poisoned_positions, poisoned_trajectories),
    )


def test_every_model_output_moves_the_loss():
    """An output the loss ignored could drift freely, so each one is perturbed
    and the loss must change.
    """
...
def test_no_step_uncertainty_starts_saturated():
    """Untrained modes must also start out distinct from one another."""
    ...
    assert not (log_standard_deviation
                == model.MINIMUM_LOG_STANDARD_DEVIATION).any()
    assert not (log_standard_deviation
                == model.MAXIMUM_LOG_STANDARD_DEVIATION).any()
    endpoints = trajectories[:, :, -1]
    assert not torch.allclose(endpoints[:, :1], endpoints, atol=1e-4)


def test_anchors_are_frozen_buffers():
    """They take no gradient and are saved inside the checkpoint."""
    predictor = model.MotionPredictor(
        reference_implementations.unit_anchor_offsets_per_type())
    assert "mode_decoder.unit_anchors" not in {
        name for name, _ in predictor.named_parameters()
    }
...
def test_pruning_conserves_probability_mass():
    """A dropped duplicate's share goes to the mode kept in its place."""
    torch.manual_seed(131)
    trajectories = torch.zeros(1, model.QUERY_COUNT, contract.FUTURE_STEPS, 2)
    trajectories[0, :, -1, 0] = 10.0 * torch.arange(model.QUERY_COUNT,
                                                    dtype=torch.float32)
    ...
    kept_trajectories, kept_logits = pruning.prune_modes_batched(
        trajectories, confidence_logits)
    ...
    assert float(confidences[0, 0]) == pytest.approx(float(probabilities[0] +
                                                           probabilities[1]),
                                                     rel=1e-6)
    assert float(confidences[0, 1:].sum()) == pytest.approx(
        1.0 - float(probabilities[0] + probabilities[1]), rel=1e-5)


def test_step_head_walks_a_line_or_an_arc():
    """Constant steps walk a straight line; steps that rotate walk an arc."""
    torch.manual_seed(137)
    decoder = model.ModeDecoder(
        reference_implementations.unit_anchor_offsets_per_type() * 0.0).eval()
    ...
    assert torch.allclose(
        steps.norm(dim=-1),
        torch.full((contract.FUTURE_STEPS - 1,), 0.5),
        atol=1e-5,
    )
