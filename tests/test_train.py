...
def test_warmup_rises_to_the_learning_rate_then_a_cosine_takes_it_to_zero():
    total_steps = 200
    warmup_steps = 20
    rates = [
        train.scheduled_learning_rate(step, warmup_steps, total_steps)
    ...
    annealing = rates[warmup_steps:]
    assert annealing[0] == train.LEARNING_RATE
    assert all(later < earlier for earlier, later in zip(annealing, annealing[1:]))
    halfway_step = warmup_steps + (total_steps - warmup_steps) // 2
    assert rates[halfway_step] == pytest.approx(0.5 * train.LEARNING_RATE)
    assert rates[total_steps] == 0.0

    resumed_process_step = 5
    assert train.scheduled_learning_rate(
        total_steps // 2, warmup_steps, total_steps, resumed_process_step
    ) == train.LEARNING_RATE * (resumed_process_step + 1) / warmup_steps
...
def test_one_training_step_runs_forward_loss_backward_and_optimizer_step():
    ...
    step_outputs = train.training_losses(predictor, batch, 1.0, 1.0, 1.0, 1.0)
...
def test_the_kernel_command_line_parses_against_the_real_training_interface(tmp_path):
    literal_arguments, _ = kernel_train_arguments()
    for weight_flag in (
