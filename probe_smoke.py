...
def main():
    ...
    predictor = MotionPredictor(model.load_anchor_file(arguments.anchors_path))
    optimizer = torch.optim.AdamW(train.parameter_groups(predictor), lr=train.LEARNING_RATE)
    scenario_paths = first_scenario_paths(arguments.staged_directory, arguments.batches * arguments.batch_size)
    ...
    for batch_index, batch in enumerate(pipeline.batches(
        ...
        for name, tensor in (("total", total), ("trajectories", trajectories), ("logits", confidence_logits)):
            if not tensor.isfinite().all():
                raise SystemExit(f"non-finite {name} at batch {batch_index}")
        optimizer.zero_grad()
        total.backward()
        gradient_norm = float(torch.nn.utils.clip_grad_norm_(predictor.parameters(), arguments.gradient_clip_norm))
        ...
        winners = metrics.mean_distance_per_mode(
            trajectories.detach(), batch["future_positions"], batch["future_mask"]
        ).argmin(dim=1)
        winner_counts.scatter_add_(0, winners, torch.ones_like(winners))
        predicted_end_norms.append(trajectories.detach()[scoreable, :, -1].norm(dim=-1).median())
        ...
        null_end_norms.append(null_trajectories[scoreable, 0, -1].norm(dim=-1).median())
        print(
